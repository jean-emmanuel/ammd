...
function BOUCLE_comptehtml_008616b16fb3d60cbbb6944d4e20989f(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'documents_liens.id_document', sql_quote($Pile[$SP]['id_document'],'','bigint(21) DEFAULT \'0\' NOT NULL')));
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/modeles/document_desc.html','html_008616b16fb3d60cbbb6944d4e20989f','_compte',36,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_compte @ ../plugins-dist/medias/modeles/document_desc.html","profiler");
	return $t0;
}


function BOUCLE_docslieshtml_008616b16fb3d60cbbb6944d4e20989f(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			...
			array('=', 'L1.objet', sql_quote(@$Pile[0]['objet'],'','VARCHAR (25) DEFAULT \'\' NOT NULL COLLATE NOCASE')), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('documents.statut',sql_quote($in)) : 
			array('=', 'documents.statut', sql_quote(@$Pile[0]['statut'],'','varchar(10) DEFAULT \'0\' NOT NULL COLLATE NOCASE')))));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/medias/modeles/document_desc.html','html_008616b16fb3d60cbbb6944d4e20989f','_docslies',8,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'
		' .
filtrer('image_graver',filtrer('image_reduire',quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), vider_url(urlencode_1738(generer_url_entite($Pile[$SP]['id_document'], 'document', '', '', true))), '', '', 60, 60, ''), '60', '60')) .
'
		<h4 class="titrem">
			' .
...
'
		<div class="actions">
			' .
BOUCLE_comptehtml_008616b16fb3d60cbbb6944d4e20989f($Cache, $Pile, $doublons, $Numrows, $SP)
. (	'
			' .
	((((((($Numrows['_compte']['total'] == '1')) AND (invalideur_session($Cache, ((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('modifier', interdire_scripts(invalideur_session($Cache, $Pile[$SP]['objet'])), invalideur_session($Cache, $Pile[$SP]['id_objet']))?" ":"")))) ?' ' :'')) AND (interdire_scripts(($Pile[$SP]['vu'] == 'non'))))  ?
	...
	'
			' .
	(($t2 = strval(invalideur_session($Cache, ((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('modifier', interdire_scripts(invalideur_session($Cache, $Pile[$SP]['objet'])), invalideur_session($Cache, $Pile[$SP]['id_objet']))?" ":""))))!=='' ?
	...
	generer_url_ecrire('document_edit',(	'id_document=' .
		$Pile[$SP]['id_document'])) .
	'" target="_blank" class="editbox">' .
	$l15 .
	'</a>
			') .
'
			' .
pipeline( 'document_desc_actions' , array('args' => array('id_document' => $Pile[$SP]['id_document'], 'position' => 'document_desc'), 'data' => '') ) .
'
		</div>
...
');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_docslies @ ../plugins-dist/medias/modeles/document_desc.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../plugins-dist/medias/modeles/document_desc.html
// Temps de compilation total: 39.937 ms
//

function html_008616b16fb3d60cbbb6944d4e20989f($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
...
'
' .
BOUCLE_docslieshtml_008616b16fb3d60cbbb6944d4e20989f($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');

	return analyse_resultat_skel('html_008616b16fb3d60cbbb6944d4e20989f', $Cache, $page, '../plugins-dist/medias/modeles/document_desc.html');
}
?>