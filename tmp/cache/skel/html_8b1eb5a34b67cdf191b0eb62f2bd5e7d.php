 ...
 * Compile :   Fri, 04 Oct 2013 12:31:40 GMT
...
.liste-objets.' .
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_objets @ ../prive/themes/spip/lists.css.html","profiler");
	return $t0;
}
...
'}');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_objets2 @ ../prive/themes/spip/lists.css.html","profiler");
	return $t0;
}
...
interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'left', null),true)) .
':1px solid #ddd;}
.plan .menu-items .item {overflow:visible;border-bottom:0;border-top:1px solid #ddd;padding-' .
interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'left', null),true)) .
':4px;}
