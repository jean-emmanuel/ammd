 ...
 * Date :      Sun, 19 Jan 2014 17:56:37 GMT
...
');
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_annonces @ ../plugins-dist/organiseur/prive/squelettes/inclure/organiseur-rappels.html","profiler");
	return $t0;
}
...
function BOUCLE_pbhtml_8a345dc969a392ec2b0898818b567a53(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
quete_condition_statut('messages.statut','!','publie',''), 
			array('=', 'messages.id_auteur', sql_quote(@$Pile[0]['id_auteur'],'','bigint(21) DEFAULT 0 NOT NULL')), 
...
');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_pb @ ../plugins-dist/organiseur/prive/squelettes/inclure/organiseur-rappels.html","profiler");
	return $t0;
}
...
');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_rv @ ../plugins-dist/organiseur/prive/squelettes/inclure/organiseur-rappels.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../plugins-dist/organiseur/prive/squelettes/inclure/organiseur-rappels.html
// Temps de compilation total: 10.019 ms
//
