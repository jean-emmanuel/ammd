...
function BOUCLE_proposerhtml_3ac52d7c91c4000e36ec35c1c40d4c83(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'articles.id_article', sql_quote($Pile[$SP]['id_article'],'','INTEGER NOT NULL')), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../prive/squelettes/contenu/article.html','html_3ac52d7c91c4000e36ec35c1c40d4c83','_proposer',29,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
');
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_proposer @ ../prive/squelettes/contenu/article.html","profiler");
	return $t0;
}
...
executer_balise_dynamique('FORMULAIRE_DATER',
	...
	array('../prive/squelettes/contenu/article.html','html_3ac52d7c91c4000e36ec35c1c40d4c83','_article',7,$GLOBALS['spip_lang'])) .
'</div>
...
executer_balise_dynamique('FORMULAIRE_EDITER_LIENS',
	...
	array('../prive/squelettes/contenu/article.html','html_3ac52d7c91c4000e36ec35c1c40d4c83','_article',10,$GLOBALS['spip_lang'])) .
'</div>
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('prive/objets/contenu/article') . ', array(\'id\' => ' . argumenter_squelette($Pile[$SP]['id_article']) . ',
	...
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . '), array("compil"=>array(\'../prive/squelettes/contenu/article.html\',\'html_3ac52d7c91c4000e36ec35c1c40d4c83\',\'\',43,$GLOBALS[\'spip_lang\']),\'ajax\' => ($v=( ' . argumenter_squelette('wysiwyg') . '))?$v:true), _request("connect"));
?'.'>
</div>
...
');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_article @ ../prive/squelettes/contenu/article.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../prive/squelettes/contenu/article.html
// Temps de compilation total: 177.010 ms
//
