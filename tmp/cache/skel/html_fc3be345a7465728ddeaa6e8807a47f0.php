...
((unique(initiale(extraire_multi($Pile[$SP]['nom']))))  ?
		...
		' . ' ' . (	'
		' .
	vide($Pile['vars'][(string)'p'] = concat(table_valeur($Pile["vars"], (string)'p', null),afficher_initiale(ancre_url(parametre_url(self(),'debutautl',(	'@' .
				$Pile[$SP]['id_auteur'])),'paginationautl'),initiale(extraire_multi($Pile[$SP]['nom'])),$Numrows['_lettre']['compteur_boucle'],table_valeur($Pile["vars"], (string)'debut', null),interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'nb', null), '10'),true))))) .
	'
		')) :
		'') .
...
// Temps de compilation total: 214.012 ms
//
...
'
' .
vide($Pile['vars'][(string)'selection'] = interdire_scripts(lister_objets_lies(entites_html(table_valeur(@$Pile[0], (string)'objet_source', null),true),interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'objet', null),true)),interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'id_objet', null),true)),interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'_objet_lien', null),true))))) .
vide($Pile['vars'][(string)'debut'] = interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'debutautl', null), interdire_scripts(eval('return '.'_request("debutautl");'.';'))),true))) .
