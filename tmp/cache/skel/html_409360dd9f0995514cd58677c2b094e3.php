 ...
 * Compile :   Fri, 04 Oct 2013 12:31:54 GMT
 * Boucles :   _rubrique
 */ 
...
function BOUCLE_rubriquehtml_409360dd9f0995514cd58677c2b094e3(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'rubriques.id_rubrique', sql_quote(@$Pile[0]['id_rubrique'],'','INTEGER NOT NULL')), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('rubriques.statut',sql_quote($in)) : 
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('prive/objets/contenu/rubrique') . ', array(\'id\' => ' . argumenter_squelette($Pile[$SP]['id_rubrique']) . ',
	...
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . '), array("compil"=>array(\'../prive/squelettes/contenu/rubrique.html\',\'html_409360dd9f0995514cd58677c2b094e3\',\'\',7,$GLOBALS[\'spip_lang\']),\'ajax\' => ($v=( ' . argumenter_squelette('wysiwyg') . '))?$v:true), _request("connect"));
?'.'>
</div>
...
');
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_rubrique @ ../prive/squelettes/contenu/rubrique.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../prive/squelettes/contenu/rubrique.html
// Temps de compilation total: 102.006 ms
//
