...
// Temps de compilation total: 35.002 ms
//
...
<div class="editer_urls">
	...
	(($t2 = strval(url_absolue(generer_url_entite(@$Pile[0]['id_objet'],interdire_scripts(@$Pile[0]['objet']),'','',interdire_scripts(eval('return '.'true'.';'))))))!=='' ?
			((	'<span class=\'link\'>
		' .
		vide($Pile['vars'][(string)'auth'] = invalideur_session($Cache, ((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('modifier', interdire_scripts(invalideur_session($Cache, @$Pile[0]['objet'])), invalideur_session($Cache, @$Pile[0]['id_objet']))?" ":""))) .
