...
/*
 * Squelette : ../plugins/auto/agenda_3_5/prive/objets/liste/evenements.html
 * Date :      Fri, 04 Oct 2013 08:05:13 GMT
 * Compile :   Fri, 04 Oct 2013 12:32:20 GMT
 * Boucles :   _liste_evt
 */ 

function BOUCLE_liste_evthtml_df15a99f38bff8b2821f4e4599fcbf9d(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$senstri = '';
	$tri = (($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'');
	if ($tri){
		$senstri = ((intval($t=(isset($Pile[0]['sens'.'_liste_evt']))?$Pile[0]['sens'.'_liste_evt']:(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1);
	...
	$command['where'] = 
			...
			array('=', 'evenements.id_evenement', sql_quote(@$Pile[0]['id_evenement'],'','INTEGER NOT NULL')))), (!(is_array(@$Pile[0]['id_article'])?count(@$Pile[0]['id_article']):strlen(@$Pile[0]['id_article'])) ? '' : ((is_array(@$Pile[0]['id_article'])) ? sql_in('evenements.id_article',sql_quote($in1)) : 
			array('=', 'evenements.id_article', sql_quote(@$Pile[0]['id_article'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), (!(is_array(@$Pile[0]['id_rubrique'])?count(@$Pile[0]['id_rubrique']):strlen(@$Pile[0]['id_rubrique'])) ? '' : ((is_array(@$Pile[0]['id_rubrique'])) ? sql_in('L1.id_rubrique',sql_quote($in2)) : 
			array('=', 'L1.id_rubrique', sql_quote(@$Pile[0]['id_rubrique'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), (!(is_array(@$Pile[0]['id_mot'])?count(@$Pile[0]['id_mot']):strlen(@$Pile[0]['id_mot'])) ? '' : ((is_array(@$Pile[0]['id_mot'])) ? sql_in('L2.id_mot',sql_quote($in3)) : 
			array('=', 'L2.id_mot', sql_quote(@$Pile[0]['id_mot'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), (!(is_array(@$Pile[0]['id_evenement_source'])?count(@$Pile[0]['id_evenement_source']):strlen(@$Pile[0]['id_evenement_source'])) ? '' : ((is_array(@$Pile[0]['id_evenement_source'])) ? sql_in('evenements.id_evenement_source',sql_quote($in4)) : 
			array('=', 'evenements.id_evenement_source', sql_quote(@$Pile[0]['id_evenement_source'],'','bigint(21) NOT NULL DEFAULT 0')))), ((@$Pile[0]["where"]) ? (@$Pile[0]["where"]) : ''), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('evenements.statut',sql_quote($in5)) : 
			array('=', 'evenements.statut', sql_quote(@$Pile[0]['statut'],'','varchar(10) DEFAULT \'0\' NOT NULL COLLATE NOCASE')))), $rech_where?$rech_where:'');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins/auto/agenda_3_5/prive/objets/liste/evenements.html','html_df15a99f38bff8b2821f4e4599fcbf9d','_liste_evt',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</td>
			' .
vide($Pile['vars'][(string)'id_evenement'] = ($Pile[$SP]['id_evenement_source'] ? $Pile[$SP]['id_evenement_source']:$Pile[$SP]['id_evenement'])) .
'<td class=\'id\'>' .
(($Pile[$SP]['id_evenement_source'])  ?
		(' ' . (	'
				' .
...
'
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_liste_evt @ ../plugins/auto/agenda_3_5/prive/objets/liste/evenements.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../plugins/auto/agenda_3_5/prive/objets/liste/evenements.html
// Temps de compilation total: 152.009 ms
//

function html_df15a99f38bff8b2821f4e4599fcbf9d($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
	...
	$connect = '';
	$page = (
(($t1 = strval(vide($Pile['vars'][(string)'defaut_tri'] = array('date_debut' => interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'date_sens', null), '1'),true)), 'titre' => '1', 'id_evenement' => '1', 'points' => '-1
'))))!=='' ?
		($t1 . '
') :
		'') .
(($t1 = BOUCLE_liste_evthtml_df15a99f38bff8b2821f4e4599fcbf9d($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
' .
	...
	<thead>
		...
		lien_ou_expose(parametre_url(self(),(($s=in_array('statut',array('>','<')))?'sens':'tri').'_liste_evt',$s?(strpos('< >','statut')-1):'statut'),(	'<span title="' .
			attribut_html(_T('public|spip|ecrire:lien_trier_statut')) .
			'">#</span>'),$s?(((intval($t=(isset($Pile[0]['sens'.'_liste_evt']))?$Pile[0]['sens'.'_liste_evt']:(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','statut')-1)):((($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')=='statut'),'ajax') .
		'</th>
			<th class=\'date\' scope=\'col\'>' .
		lien_ou_expose(parametre_url(self(),(($s=in_array('date_debut',array('>','<')))?'sens':'tri').'_liste_evt',$s?(strpos('< >','date_debut')-1):'date_debut'),_T('public|spip|ecrire:date'),$s?(((intval($t=(isset($Pile[0]['sens'.'_liste_evt']))?$Pile[0]['sens'.'_liste_evt']:(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','date_debut')-1)):((($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')=='date_debut'),'ajax') .
		'</th>
			<th class=\'article secondaire\' scope=\'col\'></th>
			<th class=\'titre principale\' scope=\'col\'>' .
		lien_ou_expose(parametre_url(self(),(($s=in_array('titre',array('>','<')))?'sens':'tri').'_liste_evt',$s?(strpos('< >','titre')-1):'titre'),_T('public|spip|ecrire:info_titre'),$s?(((intval($t=(isset($Pile[0]['sens'.'_liste_evt']))?$Pile[0]['sens'.'_liste_evt']:(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','titre')-1)):((($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')=='titre'),'ajax') .
		'</th>
			<th class=\'lieu\' scope=\'col secondaire\'>' .
		_T('agenda:evenement_lieu') .
		'</th>
			<th class=\'id\' scope=\'col\'>' .
		lien_ou_expose(parametre_url(self(),(($s=in_array('id_evenement',array('>','<')))?'sens':'tri').'_liste_evt',$s?(strpos('< >','id_evenement')-1):'id_evenement'),_T('public|spip|ecrire:info_numero_abbreviation'),$s?(((intval($t=(isset($Pile[0]['sens'.'_liste_evt']))?$Pile[0]['sens'.'_liste_evt']:(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','id_evenement')-1)):((($t=(isset($Pile[0]['tri'.'_liste_evt']))?$Pile[0]['tri'.'_liste_evt']:interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date_debut'),true)))?tri_protege_champ($t):'')=='id_evenement'),'ajax') .
		'</th>
		</tr>
	</thead>
	<tbody>
...
');
	...
	return analyse_resultat_skel('html_df15a99f38bff8b2821f4e4599fcbf9d', $Cache, $page, '../plugins/auto/agenda_3_5/prive/objets/liste/evenements.html');
}
?>