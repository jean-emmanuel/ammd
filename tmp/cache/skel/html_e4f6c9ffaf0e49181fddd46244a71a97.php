 ...
 * Compile :   Fri, 07 Feb 2014 10:28:33 GMT
...
function BOUCLE_artsrhtml_e4f6c9ffaf0e49181fddd46244a71a97(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'articles.statut', sql_quote(interdire_scripts($Pile[$SP-2]['valeur']),'','varchar(10) DEFAULT \'0\' NOT NULL COLLATE NOCASE')), sql_in('articles.id_rubrique', calcul_branche_in($Pile[$SP]['id_rubrique'])));
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_artsr @ ../prive/squelettes/inclure/accueil-information.html","profiler");
	return $t0;
}
...
quete_condition_statut('rubriques.statut','!','publie',''), 
			array('=', 'L1.id_auteur', sql_quote(interdire_scripts(invalideur_session($Cache, table_valeur($GLOBALS["visiteur_session"], (string)'id_auteur', null))),'','bigint(21) DEFAULT \'0\' NOT NULL')));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_restreints @ ../prive/squelettes/inclure/accueil-information.html","profiler");
	return $t0;
}
...
function BOUCLE_artshtml_e4f6c9ffaf0e49181fddd46244a71a97(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'articles.statut', sql_quote(interdire_scripts(safehtml($Pile[$SP]['valeur'])),'','varchar(10) DEFAULT \'0\' NOT NULL COLLATE NOCASE')));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_arts @ ../prive/squelettes/inclure/accueil-information.html","profiler");
	return $t0;
}
...
function BOUCLE_cpthtml_e4f6c9ffaf0e49181fddd46244a71a97(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_cpt @ ../prive/squelettes/inclure/accueil-information.html","profiler");
	return $t0;
}
...
function BOUCLE_autshtml_e4f6c9ffaf0e49181fddd46244a71a97(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'auteurs.statut', sql_quote(interdire_scripts($Pile[$SP]['valeur']),'','varchar(255)  DEFAULT \'0\' NOT NULL COLLATE NOCASE')));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_auts @ ../prive/squelettes/inclure/accueil-information.html","profiler");
	return $t0;
}
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_cpta @ ../prive/squelettes/inclure/accueil-information.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../prive/squelettes/inclure/accueil-information.html
// Temps de compilation total: 6.494 ms
//
