...
function BOUCLE_enlignehtml_11a92398fa9bac5f4eaedfe6bce6fc00(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('NOT', 
			array('=', 'auteurs.id_auteur', sql_quote(interdire_scripts(invalideur_session($Cache, table_valeur($GLOBALS["visiteur_session"], (string)'id_auteur', null))),'','INTEGER NOT NULL'))), sql_in('auteurs.statut',sql_quote($in)), 
			array('>', 'auteurs.en_ligne', sql_quote(date('Y-m-d H:i:s',strtotime('-15 minutes')),'','datetime DEFAULT \'0000-00-00 00:00:00\' NOT NULL')));
...
'</a>');
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_enligne @ ../prive/objets/liste/auteurs_enligne.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../prive/objets/liste/auteurs_enligne.html
// Temps de compilation total: 2.589 ms
//
