 ...
 * Compile :   Fri, 07 Feb 2014 10:28:21 GMT
 * Boucles :   _lettre, _liste_aut
 */ 

function BOUCLE_lettrehtml_b7d9266132c7de4cef0fdf5f43e450d5(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  array (
	...
	$command['where'] = 
			...
			array('=', 'auteurs.id_auteur', sql_quote(@$Pile[0]['id_auteur'],'','INTEGER NOT NULL')))), ((@$Pile[0]["where"]) ? (@$Pile[0]["where"]) : ''), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('auteurs.statut',sql_quote($in1)) : 
			array('=', 'auteurs.statut', sql_quote(@$Pile[0]['statut'],'','varchar(255)  DEFAULT \'0\' NOT NULL COLLATE NOCASE')))), $rech_where?$rech_where:'', 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../prive/objets/liste/auteurs.html','html_b7d9266132c7de4cef0fdf5f43e450d5','_lettre',18,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
vide($Numrows['_lettre']['compteur_boucle']=$iter->skip(interdire_scripts(moins(entites_html(sinon(table_valeur(@$Pile[0], (string)'nb', null), '10'),true),(($Numrows['_lettre']['compteur_boucle'] == '1') ? '2':'1'))),$Numrows['_lettre']['total'])));
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_lettre @ ../prive/objets/liste/auteurs.html","profiler");
	return $t0;
}


function BOUCLE_liste_authtml_b7d9266132c7de4cef0fdf5f43e450d5(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$command['where'] = 
			array((!(is_array(@$Pile[0]['id_auteur'])?count(@$Pile[0]['id_auteur']):strlen(@$Pile[0]['id_auteur'])) ? '' : ((is_array(@$Pile[0]['id_auteur'])) ? sql_in('auteurs.id_auteur',sql_quote($in)) : 
			array('=', 'auteurs.id_auteur', sql_quote(@$Pile[0]['id_auteur'],'','INTEGER NOT NULL')))), (!(is_array(@$Pile[0]['id_mot'])?count(@$Pile[0]['id_mot']):strlen(@$Pile[0]['id_mot'])) ? '' : ((is_array(@$Pile[0]['id_mot'])) ? sql_in('L1.id_mot',sql_quote($in1)) : 
			array('=', 'L1.id_mot', sql_quote(@$Pile[0]['id_mot'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), ((@$Pile[0]["where"]) ? (@$Pile[0]["where"]) : ''), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('auteurs.statut',sql_quote($in2)) : 
			array('=', 'auteurs.statut', sql_quote(@$Pile[0]['statut'],'','varchar(255)  DEFAULT \'0\' NOT NULL COLLATE NOCASE')))), $rech_where?$rech_where:'');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../prive/objets/liste/auteurs.html','html_b7d9266132c7de4cef0fdf5f43e450d5','_liste_aut',8,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</td>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_liste_aut @ ../prive/objets/liste/auteurs.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../prive/objets/liste/auteurs.html
// Temps de compilation total: 19.262 ms
//

function html_b7d9266132c7de4cef0fdf5f43e450d5($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
	...
	$connect = '';
	$page = (
(($t1 = strval(vide($Pile['vars'][$_zzz=(string)'defaut_tri'] = array('statut' => '1', 'multi nom' => '1', 'site' => '1', 'compteur_articles' => '-1', 'points' => '-1
'))))!=='' ?
		($t1 . '
') :
		'') .
(($t1 = BOUCLE_liste_authtml_b7d9266132c7de4cef0fdf5f43e450d5($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
' .
	...
	<thead>
		...
		(($t3 = BOUCLE_lettrehtml_b7d9266132c7de4cef0fdf5f43e450d5($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
...
');
	...
	return analyse_resultat_skel('html_b7d9266132c7de4cef0fdf5f43e450d5', $Cache, $page, '../prive/objets/liste/auteurs.html');
}
?>