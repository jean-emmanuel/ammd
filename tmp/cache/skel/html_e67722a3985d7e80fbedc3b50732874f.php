...
/*
 * Squelette : prive/modeles/pagination_page.html
 * Date :      Sun, 19 Jan 2014 17:32:59 GMT
 * Compile :   Fri, 07 Feb 2014 11:37:20 GMT
 * Boucles :   _pages
 */ 

function BOUCLE_pageshtml_e67722a3985d7e80fbedc3b50732874f(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"DATA",
		$command,
		array('prive/modeles/pagination_page.html','html_e67722a3985d7e80fbedc3b50732874f','_pages',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
(($t1 = strval(interdire_scripts((($Pile[$SP]['valeur'] < table_valeur($Pile["vars"], (string)'derniere', null)) ? table_valeur($Pile["vars"], (string)'sep', null):''))))!=='' ?
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_pages @ prive/modeles/pagination_page.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette prive/modeles/pagination_page.html
// Temps de compilation total: 21.023 ms
//

function html_e67722a3985d7e80fbedc3b50732874f($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
...
(($t1 = BOUCLE_pageshtml_e67722a3985d7e80fbedc3b50732874f($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
	' .
  ...
  class=\'label\'>' . $t3 . '</span>') :
	...
	class=\'sep\'>' . $t4 . '</span>') :
		...
		>')) :
				'')) . $t1 . (	(($t3 = strval(((table_valeur($Pile["vars"], (string)'derniere', null) < interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'nombre_pages', null),true))) ? interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'nombre_pages', null),true)):'')))!=='' ?
				((	'<span
	...
	rel=\'nofollow\'>') . $t3 . '</a>') :
				'') .
		'</span>' .
		(($t3 = strval(vide($Pile['vars'][$_zzz=(string)'item'] = interdire_scripts(mult(entites_html(table_valeur(@$Pile[0], (string)'page_courante', null),true),interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'pas', null),true)))))))!=='' ?
				('
	' . $t3 . '
	') :
				'') .
...
')) :
	...
	return analyse_resultat_skel('html_e67722a3985d7e80fbedc3b50732874f', $Cache, $page, 'prive/modeles/pagination_page.html');
}
?>