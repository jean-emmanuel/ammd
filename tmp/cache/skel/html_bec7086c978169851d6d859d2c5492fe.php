...
function BOUCLE_illustrationshtml_bec7086c978169851d6d859d2c5492fe(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			...
			array('=', 'L2.id_objet', sql_quote(@$Pile[0]['id_objet'],'','bigint(21) DEFAULT \'0\' NOT NULL')), 
			array('=', 'L2.objet', sql_quote(@$Pile[0]['objet'],'','VARCHAR (25) DEFAULT \'\' NOT NULL COLLATE NOCASE')), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('documents.statut',sql_quote($in)) : 
...
'
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_illustrations @ ../plugins-dist/medias/prive/squelettes/inclure/portfolio-documents.html","profiler");
	return $t0;
}
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_portfolio @ ../plugins-dist/medias/prive/squelettes/inclure/portfolio-documents.html","profiler");
	return $t0;
}
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_documents @ ../plugins-dist/medias/prive/squelettes/inclure/portfolio-documents.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../plugins-dist/medias/prive/squelettes/inclure/portfolio-documents.html
// Temps de compilation total: 187.010 ms
//
