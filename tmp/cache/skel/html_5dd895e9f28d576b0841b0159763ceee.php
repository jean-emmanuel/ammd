 ...
 * Date :      Fri, 04 Oct 2013 08:08:39 GMT
...
function BOUCLE_liste_brehtml_5dd895e9f28d576b0841b0159763ceee(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$command['where'] = 
			...
			array('=', 'breves.id_breve', sql_quote(@$Pile[0]['id_breve'],'','INTEGER NOT NULL')))), (!(is_array(@$Pile[0]['id_secteur'])?count(@$Pile[0]['id_secteur']):strlen(@$Pile[0]['id_secteur'])) ? '' : ((is_array(@$Pile[0]['id_secteur'])) ? sql_in('breves.id_rubrique',sql_quote($in1)) : 
			array('=', 'breves.id_rubrique', sql_quote(@$Pile[0]['id_secteur'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), (!(is_array(@$Pile[0]['id_mot'])?count(@$Pile[0]['id_mot']):strlen(@$Pile[0]['id_mot'])) ? '' : ((is_array(@$Pile[0]['id_mot'])) ? sql_in('L1.id_mot',sql_quote($in2)) : 
			array('=', 'L1.id_mot', sql_quote(@$Pile[0]['id_mot'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), (!(is_array(@$Pile[0]['id_rubrique'])?count(@$Pile[0]['id_rubrique']):strlen(@$Pile[0]['id_rubrique'])) ? '' : ((is_array(@$Pile[0]['id_rubrique'])) ? sql_in('breves.id_rubrique',sql_quote($in3)) : 
			array('=', 'breves.id_rubrique', sql_quote(@$Pile[0]['id_rubrique'],'','bigint(21) DEFAULT \'0\' NOT NULL')))), ((@$Pile[0]["where"]) ? (@$Pile[0]["where"]) : ''), $rech_where?$rech_where:'', (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('breves.statut',sql_quote($in4)) : 
			array('=', 'breves.statut', sql_quote(@$Pile[0]['statut'],'','varchar(6)  DEFAULT \'0\' NOT NULL COLLATE NOCASE')))));
...
'</td>
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_liste_bre @ ../plugins-dist/breves/prive/objets/liste/breves.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette ../plugins-dist/breves/prive/objets/liste/breves.html
// Temps de compilation total: 144.008 ms
//
