 ...
 * Date :      Sun, 19 Jan 2014 17:33:02 GMT
 * Compile :   Fri, 07 Feb 2014 11:39:52 GMT
 * Boucles :   _art
 */ 

function BOUCLE_arthtml_990c0c9db89919331256b7a08ec1e413(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'articles.id_article', sql_quote(@$Pile[0]['id_article'],'','INTEGER NOT NULL')), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('articles.statut',sql_quote($in)) : 
			array('=', 'articles.statut', sql_quote(@$Pile[0]['statut'],'','varchar(10) DEFAULT \'0\' NOT NULL COLLATE NOCASE')))));
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('notifications/article_publie.html','html_990c0c9db89919331256b7a08ec1e413','_art',8,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'
' .
vide($Pile['vars'][$_zzz=(string)'auteurs'] = recuperer_fond('modeles/lesauteurs', array('objet'=>'article','id_objet' => $Pile[$SP]['id_article'],'id_article' => $Pile[$SP]['id_article']), array('trim'=>true, 'compil'=>array('notifications/article_publie.html','html_990c0c9db89919331256b7a08ec1e413','_art',0,$GLOBALS['spip_lang'])), '')) .
...
');
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_art @ notifications/article_publie.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette notifications/article_publie.html
// Temps de compilation total: 4.927 ms
//

function html_990c0c9db89919331256b7a08ec1e413($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
...
'<'.'?php header("' . (	'Content-type: text/plain' .
	(($t2 = strval(interdire_scripts($GLOBALS['meta']['charset'])))!=='' ?
			...
			'')) . '"); ?'.'>
' .
BOUCLE_arthtml_990c0c9db89919331256b7a08ec1e413($Cache, $Pile, $doublons, $Numrows, $SP) .
'
' .
'<' . '?php header("X-Spip-Filtre: '.'supprimer_tags|filtrer_entites|trim' . '"); ?'.'>');

	return analyse_resultat_skel('html_990c0c9db89919331256b7a08ec1e413', $Cache, $page, 'notifications/article_publie.html');
}
?>