...
// Temps de compilation total: 7.530 ms
//
...
(($t1 = strval(interdire_scripts(((entites_html(table_valeur(@$Pile[0], (string)'nom', null),true)) ?' ' :''))))!=='' ?
	...
	vide($Pile['vars'][$_zzz=(string)'readonly'] = interdire_scripts(((((entites_html(table_valeur(@$Pile[0], (string)'readonly', null),true)) AND ((	interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'readonly', null),true)) .
			'!={non}'))) ?' ' :'') ? 'readonly':''))) .
	...
	(($t2 = strval(interdire_scripts(((in_array(entites_html(table_valeur(@$Pile[0], (string)'type_saisie', null),true),table_valeur($Pile["vars"], (string)'saisies_autonomes', null))) ?'' :' '))))!=='' ?
			($t2 . (	'
		' .
		vide($Pile['vars'][$_zzz=(string)'erreurs'] = table_valeur(table_valeur(@$Pile[0], (string)'erreurs', null),interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'nom', null),true)))) .
		vide($Pile['vars'][$_zzz=(string)'li_class'] = interdire_scripts(((substr(entites_html(table_valeur(@$Pile[0], (string)'type_saisie', null),true),'0','9') == 'selecteur') ? 'selecteur_item':''))) .
