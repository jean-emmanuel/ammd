 ...
 * Compile :   Fri, 04 Oct 2013 12:21:23 GMT
 * Boucles :   _document
 */ 

function BOUCLE_documenthtml_acd6c646362c1be2aa249ca2d7c42bd8(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			...
			array('=', 'documents.id_document', sql_quote(@$Pile[0]['id_document'],'','INTEGER NOT NULL')), (!(is_array(@$Pile[0]['mode'])?count(@$Pile[0]['mode']):strlen(@$Pile[0]['mode'])) ? '' : ((is_array(@$Pile[0]['mode'])) ? sql_in('documents.mode',sql_quote($in)) : 
			array('=', 'documents.mode', sql_quote(@$Pile[0]['mode'],'','varchar(10) DEFAULT \'document\' NOT NULL COLLATE NOCASE')))));
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('plugins-dist/medias/modeles/img.html','html_acd6c646362c1be2aa249ca2d7c42bd8','_document',1,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
' .
vide($Pile['vars'][(string)'image'] = interdire_scripts(((((($a = match(entites_html(sinon(table_valeur(@$Pile[0], (string)'mode_force', null), interdire_scripts($Pile[$SP]['mode'])),true),'image|vignette')) OR (is_string($a) AND strlen($a))) ? $a : interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'embed', null),true)))) ?' ' :''))) .
...
(!(table_valeur($Pile["vars"], (string)'image', null))  ?
		(' ' . (	'
	' .
	vide($Pile['vars'][(string)'fichier'] = extraire_attribut(quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), '', '', '', 0, 0, ''),'src')) .
	'
	' .
	vide($Pile['vars'][(string)'width'] = extraire_attribut(quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), '', '', '', 0, 0, ''),'width')) .
	'
	' .
	vide($Pile['vars'][(string)'height'] = extraire_attribut(quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), '', '', '', 0, 0, ''),'height')) .
	'
<span class=\'spip_document_' .
	$Pile[$SP]['id_document'] .
	' spip_documents' .
...
')) :
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_document @ plugins-dist/medias/modeles/img.html","profiler");
	return $t0;
}

//
// Fonction principale du squelette plugins-dist/medias/modeles/img.html
// Temps de compilation total: 158.009 ms
//

function html_acd6c646362c1be2aa249ca2d7c42bd8($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
	...
	$connect = '';
	$page = (
BOUCLE_documenthtml_acd6c646362c1be2aa249ca2d7c42bd8($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');

	return analyse_resultat_skel('html_acd6c646362c1be2aa249ca2d7c42bd8', $Cache, $page, 'plugins-dist/medias/modeles/img.html');
}
?>